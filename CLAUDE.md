...
**日志**：API 入口统一通过 `withApiLogging` 输出结构化 JSON，默认控制台输出。Docker 镜像默认 `KANBAN_LOG_DIR=/data/logs`，会写 `/data/logs/kanban.log`；`KANBAN_LOG_FILE_ENABLED=false` 可关闭文件日志。文件日志支持按大小滚动、保留文件数和保留天数清理。
